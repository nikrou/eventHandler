...
class rsExtEventHandlerPublic extends rsExtPost
{
    ...
    public static function getIcalVEVENT($rs)
    {
        ...
        $l[] = "URL:" . $rs->getURL();
        $l[] = "UID:" . $rs->post_id;
        $l[] = "DTSTAMP;TZID=" . dcCore::app()->blog->settings->system->blog_timezone . ":" . dt::dt2str("%Y%m%dT%H%M%S", $rs->post_upddt, $rs->post_tz);
        $l[] = "DTSTART;TZID=" . dcCore::app()->blog->settings->system->blog_timezone . ":" . dt::dt2str("%Y%m%dT%H%M%S", $rs->event_startdt);
        $l[] = "DTEND;TZID=" . dcCore::app()->blog->settings->system->blog_timezone . ":" . dt::dt2str("%Y%m%dT%H%M%S", $rs->event_enddt);
